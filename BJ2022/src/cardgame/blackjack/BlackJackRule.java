...
public class BlackJackRule {
	...
	/**
	 * A(エース)を11または1として計算(11：真、1：偽)
	 */
	private static boolean isSoft;
	...
	/**
	 * 手札の合計を計算する。及び、soft hand か hard hand かを判断する。
	 * 
	 * @param hand 手札
	 * @return 手札の合計
	 */
	public static int totalHand(Hand hand) {
		...
		for (Card card : hand.getHand()) {
			if (card.getCardNumber() >= 10) {
				cardTotal += 10;
			// Aceは一旦hardでカウント（+1）しておく
			} else if (card.getCardNumber() == 1) {
				cardTotal += 1;
				hasAce = true;
			} else {
				cardTotal += card.getCardNumber();
			}
		}

		isSoft = false;
		// Ace が2枚以上あってもcardTotalが21以下でAceを11とカウントするのは1回のみ
		if (cardTotal + 10 <= 21 && hasAce) {
			cardTotal += 10;
			isSoft = true;
		}
