...
/**
 * クラスNMojiShiritoriPlayerのフィールド，コンストラクタの定義が完成したら実行すること
 * @author handa
 *
 */
public class TestDrive13NMojiShiritoriPlayer {
	/**
	 * テスト対象のクラスはlesson05b.shiritori.NMojiShiritoriPlayer
	 */
	private static Class<?> target = lesson05b.shiritori.NMojiShiritoriPlayer.class;
	...
	public static void main(String[] args) {
		...
		コンストラクタ動作検査();
		System.out.println("メソッドupdateの検査");
		メソッドupdate動作検査();
		System.out.println("全部で37個のOKが出ていたら次に進んでください: " + okCount);
	...
	public static void コンストラクタ動作検査() {
		Constructor<lesson05b.shiritori.NMojiShiritoriPlayer> con = (Constructor<lesson05b.shiritori.NMojiShiritoriPlayer>) ConstructorCheck.getConstructor(target, new Class<?>[]{String.class, String[].class, int.class});
		if (con != null) {
			try {
				String[] sTest = {"りんご", "ごりら"};
				lesson05b.shiritori.NMojiShiritoriPlayer e = con.newInstance("hi", sTest, 2);
				...
				e = con.newInstance("hi2", sTest2, 2);
				...
				printResult(sTest2 == vocabulary, "フィールドvocabularyに値が格納されている？");
				printResult(vocabulary != null && "りんご".equals(vocabulary[0]), "vocabulary[0]の値を書き換えていない？");
				printResult(vocabulary != null && "ごりら".equals(vocabulary[1]), "vocabulary[1]の値を書き換えていない？");
		...
		if (con != null) {
			try {
				...
				if (m != null) {
					...
					printResult("ぬ".equals(FieldCheck.getSuperClassPrivateField(e, "keyChars")), "フィールドkeyCharsは「ぬ」？");
					m.invoke(e, new Object[]{"あ"});
					printResult("あ".equals(FieldCheck.getSuperClassPrivateField(e, "keyChars")), "フィールドkeyCharsは「あ」？");
