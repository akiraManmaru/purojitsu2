...
public class TestDrive15SaigokaraNiMojiShiritori {
	/**
	 * テスト対象のクラスはlesson05b.SaigokaraNiMojiShiritori
	 */
	private static Class<?> target = lesson05b.shiritori.SaigokaraNiMojiShiritori.class;
	...
	private static void クラスの修飾子と継承実装関係の検査() {
		...
		printResult(target.getSuperclass().getName().equals("lesson05b.shiritori.Game"), "Gameを継承している?");
	...
	private static void コンストラクタ動作検査() {
		Constructor<lesson05b.shiritori.SaigokaraNiMojiShiritori> con = (Constructor<lesson05b.shiritori.SaigokaraNiMojiShiritori>) ConstructorCheck.getConstructor(target, new Class<?>[]{String[].class, String[][].class});
		if (con != null) {
			try {
				String[] sTest = {"近大3", "情報3"};
				String[][] sTest2 = {new String[]{"りんご", "らくてん"}, new String[]{"ごりら"}};
				lesson05b.shiritori.SaigokaraNiMojiShiritori e = con.newInstance(sTest, sTest2);
				String s = (String) FieldCheck.getSuperClassPrivateField(e, "initialWord");
				printResult("最後から２文字しりとり".equals(s), "フィールドinitialWordに値がセットされている?");
				...
				if (r != null) {
					printResult(r instanceof lesson05b.shiritori.NMojiShiritoriRule, "フィールドruleはNMojiShiritoriRuleクラスのオブジェクト？");
					int pos = (int)  FieldCheck.getPrivateField(r, "position");
					printResult(pos == 2, "フィールドruleのフィールドpositionは2？");
				}
				else {
				...
				if (players != null) {
					...
					printResult("近大3".equals(player.getName()), "一人目の名前は近大？");
					printResult(player instanceof lesson05b.shiritori.NMojiShiritoriPlayer, "一人目はNtoriPlayerはのオブジェクト？");
					printResult(((String[]) FieldCheck.getSuperClassPrivateField(player, "vocabulary")).length == 2, "一人目の単語数は2?");
					printResult(((int) FieldCheck.getPrivateField(player, "position")) == 2, "一人目のフィールドpositionは2?");

					player = players.get(1);
					printResult("情報3".equals(player.getName()), "二人目の名前は情報3？");
					printResult(player instanceof lesson05b.shiritori.NMojiShiritoriPlayer, "二人目はNtoriPlayerはのオブジェクト？");
					printResult(((String[]) FieldCheck.getSuperClassPrivateField(player, "vocabulary")).length == 1, "二人目の単語数は1?");
					printResult(((int) FieldCheck.getPrivateField(player, "position")) == 2, "二人目のフィールドpositionは2?");
