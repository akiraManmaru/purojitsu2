...
public class TestDrive09Shiritori {
	/**
	 * テスト対象のクラスはlesson05b.shiritori.Shiritori
	 */
	private static Class<?> target = lesson05b.shiritori.Shiritori.class;
	...
	private static void コンストラクタ動作検査() {
		Constructor<lesson05b.shiritori.Shiritori> con = (Constructor<lesson05b.shiritori.Shiritori>) ConstructorCheck.getConstructor(target, new Class<?>[]{String[].class, String[][].class});
		if (con != null) {
			try {
				String[] sTest = {"近大", "情報"};
				String[][] sTest2 = {new String[]{"りんご", "らくてん"}, new String[]{"ごりら"}};
				lesson05b.shiritori.Shiritori e = con.newInstance(sTest, sTest2);
				String s = (String) FieldCheck.getSuperClassPrivateField(e, "initialWord");
				printResult("しりとり".equals(s), "フィールドinitialWordに値がセットされている?");
				...
				if (r != null) {
					...
					printResult(len == 1, "フィールドruleのフィールドlengthは1？");
				}
				else {
				...
				if (players != null) {
					...
					printResult("近大".equals(player.getName()), "一人目の名前は近大？");
					...
					printResult(((int) FieldCheck.getPrivateField(player, "length")) == 1, "一人目のフィールドlengthは1?");

					player = players.get(1);
					printResult("情報".equals(player.getName()), "二人目の名前は情報？");
					...
					printResult(((int) FieldCheck.getPrivateField(player, "length")) == 1, "二人目のフィールドlengthは1?");
