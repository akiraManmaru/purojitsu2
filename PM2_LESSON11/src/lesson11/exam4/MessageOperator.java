...
public class MessageOperator implements MsgOpInterface {
	...
	public void processMessage(String message, ClassMember sender) {
		if (sender instanceof Student || sender instanceof TA) {
	...
	public void processReport(String message, ClassMember sender) {
		String m = sender.getName();
		if (sender instanceof Student) {
			m = ((Student) sender).getNumber();
		}

		for (ClassMember cm : members) {
			if (cm instanceof Lecturer || cm instanceof TA) {
				cm.receiveMessage(makeMessageStr(message, m));
			}
		}
